...
public final class InstanceRepository: @unchecked Sendable {
    ...
    public func save(_ instance: WeChatInstance) throws {
        let sql = "INSERT OR REPLACE INTO instances (id,name,bundle_id,app_path,version,status,created_at) VALUES (?,?,?,?,?,?,?);"
    ...
    public func all() throws -> [WeChatInstance] {
        var statement: OpaquePointer?; defer { sqlite3_finalize(statement) }
        guard sqlite3_prepare_v2(database.handle, "SELECT id,name,bundle_id,app_path,version,status,created_at FROM instances ORDER BY created_at DESC;", -1, &statement, nil) == SQLITE_OK else { throw WeBoxError.commandFailed("无法读取实例。") }
        ...
        while sqlite3_step(statement) == SQLITE_ROW {
            ...
            result.append(WeChatInstance(id: id, name: value(1), bundleIdentifier: value(2), appPath: value(3), version: value(4), status: InstanceStatus(rawValue: value(5)) ?? .error, createdAt: Date(timeIntervalSince1970: sqlite3_column_double(statement, 6))))
        }
        return result
    }
